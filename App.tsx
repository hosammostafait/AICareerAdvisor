...
const App: React.FC = () => {
  ...
  const handleFormSubmit = async (data: UserInput) => {
    ...
    try {
      ...
      console.error(err);
      setError('حدث خطأ أثناء التواصل مع المساعد الذكي. يرجى التأكد من اتصالك بالإنترنت والمحاولة مرة أخرى.');
      setAppState('error');
    }
  };
    ...
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${darkMode ? 'dark bg-slate-900' : 'bg-slate-50'}`}>
      ...
      <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-50 no-print transition-colors">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          <div 
            className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 cursor-pointer hover:opacity-80 transition-opacity"
            onClick={handleReset}
          >
          ...
          <div className="flex items-center gap-3">
             ...
             <div className="text-xs font-semibold bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-600">
               نسخة تجريبية
             </div>
          </div>
        </div>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 py-8 md:py-12 flex flex-col items-center justify-center">
        ...
        {appState === 'loading' && (
          <div className="flex flex-col items-center justify-center py-20 text-center animate-pulse">
             <div className="w-20 h-20 bg-indigo-100 dark:bg-indigo-900 rounded-full flex items-center justify-center mb-6 relative">
                <BrainIcon className="w-10 h-10 text-indigo-600 dark:text-indigo-400 animate-bounce" />
             ...
             <p className="text-slate-500 dark:text-slate-400 max-w-sm">يقوم الذكاء الاصطناعي الآن بتحليل مهنتك والبحث عن أفضل الأدوات والمصادر لك.</p>
          </div>
        )}
        ...
        {appState === 'error' && (
           <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-lg text-center max-w-md border border-red-100 dark:border-red-900/30">
              ...
              <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">عفواً، حدث خطأ</h3>
              <p className="text-slate-600 dark:text-slate-400 mb-6">{error}</p>
              <button 
                onClick={() => setAppState('input')}
                className="w-full bg-slate-800 dark:bg-slate-700 text-white py-3 rounded-xl hover:bg-slate-900 dark:hover:bg-slate-600 transition-colors font-semibold"
              >
                حاول مرة أخرى
              </button>
           </div>
      ...
      {/* Footer */}
      <footer className="bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 py-6 mt-auto no-print transition-colors">
        <div className="max-w-5xl mx-auto px-4 text-center text-slate-500 dark:text-slate-400 text-sm">
            <p className="mb-2">
                ...
                >
                    مبادرة الذكاء الاصطناعي ببساطة 2025
                </a>
                {' - '}
                برومبت وتصميم
                {' '}
